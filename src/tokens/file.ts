...
export async function expandFileTokens(
  ...
  while (true) {
    ...
    if (args.size > 0) {
      logger?.log(`file: ${rawPath} ${formatArgsForCall(args)} → ${resolved} (${args.size} args: ${formatArgsForLog(args)})`)
    }
  ...
  try {
    ...
    for (const configDir of configDirs) {
      ...
      try {
        ...
        logger?.log(`file: ${token} → ${configResolved} (${content.length} chars) [config dir fallback]`)
    ...
    logger?.log(`file: ${token} → ${resolved} DOES NOT EXIST (tried ${configDirs.length} fallback(s))`)
    recordDiagnostic(ctx, {
      kind: "missing-file",
      token,
      rawPath,
...
async function recursivelyExpand(
  ...
  logger?.log(`file: ${token} → ${resolved} recursive expansion (${expanded.length} chars, depth ${ctx.depth + 1})`)
  return expanded
}
