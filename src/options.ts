...
export function resolveMdExpandOptions(
  ...
  const configDirs = (merged.configDirs && merged.configDirs.length)
    ? merged.configDirs.map(p => path.resolve(p))
    : []
  const maxDepth = typeof merged.maxDepth === "number" && Number.isFinite(merged.maxDepth) && merged.maxDepth >= 0
    ? Math.floor(merged.maxDepth)
    : MAX_DEPTH
  const debug = merged.debug === true || process.env.FILE_INTERP_DEBUG === "1" || process.env.MD_EXPAND_DEBUG === "1" || process.env.OPENCODE_PLUGIN_MD_EXPAND_DEBUG === "1"
  const logDir = typeof merged.logDir === "string" && merged.logDir.length
